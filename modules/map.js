...
document.addEventListener('DOMContentLoaded', function() {
    ...
    mapRegions.forEach(region => {
        ...
        region.addEventListener('mouseleave', function() {
            ...
            setTimeout(() => {
                // 检查鼠标是否真的离开了整个地图区域
                const activeRegion = document.querySelector('.map-region:hover');
                if (!activeRegion) {
                    hideTooltip();
                }
            }, 300);
    ...
    exploreBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // 提取地区标识 - 注意：northeast 需要在 north 之前检查！
            let regionId = '';
            if (cardId.includes('northeast')) regionId = 'northeast';
            else if (cardId.includes('north')) regionId = 'north';
            else if (cardId.includes('south')) regionId = 'south';
            else if (cardId.includes('west')) regionId = 'west';
            else if (cardId.includes('east')) regionId = 'east';
            else if (cardId.includes('central')) regionId = 'central';

            // 跳转到3D风景展示页面，并传递地区参数
            if (regionId) {
                window.location.href = `scenery.html?region=${regionId}`;
            }
        });
    });
});
